...
module RS
  ...
  # 폰트 리스트 예:) ["나눔고딕, "굴림"]
  # 폰트가 시스템에 설치되어있거나 Fonts 폴더에 있어야 합니다.
...
class Window_Message < Window_Selectable  
  ...
  def convert_escape_characters(text)
    ...
    text.gsub!(RS::CODE["말풍선"]) do
      if $game_temp.word_wrap_enabled
        raise ["말풍선 모드가 활성화되어있을 때에는",
          "자동 개행 기능을 사용할 수 없습니다",
          "스크립트 호출 RS::LIST[\"자동개행\"] = false로",
          "자동 개행 기능을 잠시 해제해주세요"].join "\n"
      end
      $game_temp.balloon = $1.to_i
      ""
    end
    text
  end
